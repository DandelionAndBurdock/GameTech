...
void Jelly::AddToScene(Scene* s) {
	...
	for (int row = 1; row < dimension; ++row) {
		for (int col = 0; col < dimension; ++col) {
			...
			GameObject* obj = CommonUtils::BuildSphereObject(
				"",
				Vector3((dimension - 1) * length, col * length, row * length),
				0.1f,
				true,
				invMass,
				false,
	...
	//Add diagonal and square distance constraints
	numNodes = dimension * dimension - dimension; // Already have some of our nodes in the frontface-layer
	for (int i = 0; i < numNodes; ++i) {
		// Horizontal constraints
		if (i < numNodes - dimension) { // Not for last column
			DistanceConstraint* constraint = new DistanceConstraint(
	...
	for (int i = 1; i <= dimension; ++i) {
		// Horizontal
		DistanceConstraint* constraint = new DistanceConstraint(
			spheres[dimension * dimension - i]->Physics(),					//Physics Object A
			spheresLF[numNodes - i]->Physics(),					//Physics Object B
			spheres[dimension * dimension - i]->Physics()->GetPosition(),	//Attachment Position on Object A	-> Currently the centre
			spheresLF[numNodes - i]->Physics()->GetPosition());	//Attachment Position on Object B	-> Currently the centre  
		...
		if (i > 1) {
			DistanceConstraint* constraint = new DistanceConstraint(
				spheres[dimension * dimension - i + 1]->Physics(),					//Physics Object A
				spheresLF[dimension - i]->Physics(),					//Physics Object B
				spheres[dimension * dimension - i + 1]->Physics()->GetPosition(),	//Attachment Position on Object A	-> Currently the centre
				spheresLF[dimension - i ]->Physics()->GetPosition());	//Attachment Position on Object B	-> Currently the centre  

			constraintsLF.push_back(constraint);

			constraint = new DistanceConstraint(
				spheres[dimension * dimension - i + 1]->Physics(),					//Physics Object A
				spheresLF[dimension - i + 1]->Physics(),					//Physics Object B
				spheres[dimension * dimension - i + 1]->Physics()->GetPosition(),	//Attachment Position on Object A	-> Currently the centre
				spheresLF[dimension - i + 1]->Physics()->GetPosition());	//Attachment Position on Object B	-> Currently the centre  
